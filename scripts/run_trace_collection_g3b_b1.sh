...
# as traces_g3b_3000 / traces_g3b_b4_3000, but with a DISCLOSED budget of 1 running step
# (1 plan-review round, max plan length 1), no wiki. Teacher on the FAU ->
...
#
# Usage:  tmux new-session -d -s trace_g3b_b1 'bash scripts/run_trace_collection_g3b_b1.sh'

set -u
cd "$(dirname "$0")/.."
source .venv/bin/activate

OUT=data/simulation_output/traces_g3b_b1_3000
...
echo "[$(date -u '+%F %T')] trace collection b1 start | GPUs=$GPUS | N=$N | out=$OUT"
...
python scripts/run_batched_parallel.py \
  ...
  --budget 1 --planning-steps 1 --max-plan-steps 1 \
  --workflow hotpot_teacher_guided_b1_plan_review \
  --questions "$QUESTIONS" --corpus "$CORPUS" \
  --out-root "$OUT" --tag tr3kb1 \
  > "$OUT/runner.log" 2>&1
RC=$?
...
echo "[$(date -u '+%F %T')] TRACE COLLECTION B1 COMPLETE"
