...
# as traces_g3b_3000 / traces_g3b_b4_3000, but with a HIDDEN budget of 5 running steps
# (1 plan-review round, max plan length 5), no wiki. Teacher on the FAU ->
...
#
# Usage:  tmux new-session -d -s trace_g3b_b5 'bash scripts/run_trace_collection_g3b_b5.sh'

set -u
cd "$(dirname "$0")/.."
source .venv/bin/activate

OUT=data/simulation_output/traces_g3b_b5_3000
...
echo "[$(date -u '+%F %T')] trace collection b5 start | GPUs=$GPUS | N=$N | out=$OUT"
...
# --hidden-budget: the student is NOT told how many steps it has.
python scripts/run_batched_parallel.py \
  ...
  --budget 5 --planning-steps 1 --max-plan-steps 5 --hidden-budget \
  --workflow hotpot_teacher_guided_b5_plan_review \
  --questions "$QUESTIONS" --corpus "$CORPUS" \
  --out-root "$OUT" --tag tr3kb5 \
  > "$OUT/runner.log" 2>&1
RC=$?
...
echo "[$(date -u '+%F %T')] TRACE COLLECTION B5 COMPLETE"
