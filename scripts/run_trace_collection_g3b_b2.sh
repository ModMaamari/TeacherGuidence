#!/usr/bin/env bash
# Trace collection v3: granite4.1:3b student, same 3000 HotpotQA train-split questions
# as traces_g3b_3000 / traces_g3b_b4_3000, but with a DISCLOSED budget of 2 running steps
# (1 plan-review round, max plan length 2), no wiki. Teacher on the FAU ->
# OpenRouter-free -> OpenRouter-paid fallback router with hard wall-clock timeouts.
# Top 4 free GPUs, 8 parallel workers each (32 concurrent).
#
...
#
# Usage:  tmux new-session -d -s trace_g3b_b2 'bash scripts/run_trace_collection_g3b_b2.sh'

set -u
cd "$(dirname "$0")/.."
source .venv/bin/activate

OUT=data/simulation_output/traces_g3b_b2_3000
...
echo "[$(date -u '+%F %T')] trace collection b2 start | GPUs=$GPUS | N=$N | out=$OUT"
...
# No --hidden-budget: the student is told its remaining budget.
python scripts/run_batched_parallel.py \
  ...
  --budget 2 --planning-steps 1 --max-plan-steps 2 \
  --workflow hotpot_teacher_guided_b2_plan_review \
  --questions "$QUESTIONS" --corpus "$CORPUS" \
  --out-root "$OUT" --tag tr3kb2 \
  > "$OUT/runner.log" 2>&1
RC=$?
...
echo "[$(date -u '+%F %T')] TRACE COLLECTION B2 COMPLETE"
