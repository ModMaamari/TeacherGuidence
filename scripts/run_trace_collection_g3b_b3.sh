...
# as traces_g3b_3000 / traces_g3b_b4_3000, but with a DISCLOSED budget of 3 running steps
# (1 plan-review round, max plan length 3), no wiki. Teacher on the FAU ->
...
#
# Usage:  tmux new-session -d -s trace_g3b_b3 'bash scripts/run_trace_collection_g3b_b3.sh'

set -u
cd "$(dirname "$0")/.."
source .venv/bin/activate

OUT=data/simulation_output/traces_g3b_b3_3000
...
echo "[$(date -u '+%F %T')] trace collection b3 start | GPUs=$GPUS | N=$N | out=$OUT"
...
echo "[$(date -u '+%F %T')] TRACE COLLECTION B3 COMPLETE"
