#!/usr/bin/env bash
# Trace collection v2: granite4.1:3b student, same 3000 HotpotQA train-split questions
# as traces_g3b_3000, but with a DISCLOSED budget of 4 running steps (3 plan-review
# rounds, max plan length 4), no wiki. Teacher on the FAU -> OpenRouter-free ->
# OpenRouter-paid fallback router with hard wall-clock timeouts. Top 4 free GPUs,
# 8 parallel workers each (32 concurrent).
#
...
#
# Usage:  tmux new-session -d -s trace_g3b_b4 'bash scripts/run_trace_collection_g3b_b4.sh'

set -u
cd "$(dirname "$0")/.."
source .venv/bin/activate

OUT=data/simulation_output/traces_g3b_b4_3000
...
echo "[$(date -u '+%F %T')] trace collection b4 start | GPUs=$GPUS | N=$N | out=$OUT"
...
echo "[$(date -u '+%F %T')] TRACE COLLECTION B4 COMPLETE"
