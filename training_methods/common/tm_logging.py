"""Shared logging + timestamped-artifact helpers for all training methods.

Every script in training_methods/ uses ``setup_logger`` (timestamped console + file
logging) and ``timestamped_dir`` (per-run artifact directories named
``<base>/<UTC timestamp>_<tag>/``) so runs never overwrite each other and every log
line carries a wall-clock timestamp.
"""
