...
OpenAI-compatible server instead of running HF ``model.generate`` in-process:
  ...
  * continuous batching on the server: N concurrent episodes decode together,
    so per-episode latency barely grows with concurrency (the b-run/Ollama
    serving model, but bf16 and with LoRA adapters served unmerged);
...
Start a server with ``training_methods/common/serve_vllm.sh``; check readiness
